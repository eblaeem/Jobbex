...
export class RefreshTokenService {
  ...
  invalidateCurrentTabId() {
    ...
    const currentTabId = this.commonService.getCurrentTabId();
  ...
  private refreshToken(isAuthUserLoggedIn: boolean) {
    ...
    return this.http
      .post(`${this.appConfig.apiEndpoint}/user/${this.apiConfigService.configuration.refreshTokenPath}`,
  ...
  private setRefreshTokenTimerStarted(): void {
    this.browserStorageService.setLocal(this.refreshTokenTimerCheckId,
      {
        isStarted: true,
        tabId: this.commonService.getCurrentTabId()
      });
  }
