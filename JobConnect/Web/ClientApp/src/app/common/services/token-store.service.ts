...
export class TokenStoreService {
  ...
  getDecodedAccessToken(): any {
    return jwt_decode.default(this.getRawAuthToken(AuthTokenType.AccessToken));
  }
  ...
  setToken(tokenType: AuthTokenType, tokenValue: string): void {
    if (this.commonService.isEmptyString(tokenValue)) {
      console.error(`${AuthTokenType[tokenType]} is null or empty.`);
    }

    if (tokenType === AuthTokenType.AccessToken && this.commonService.isEmptyString(tokenValue)) {
  ...
  hasStoredAccessAndRefreshTokens(): boolean {
    ...
    return !this.commonService.isEmptyString(accessToken) && !this.commonService.isEmptyString(refreshToken);
  }
}
